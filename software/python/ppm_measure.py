...
from ppm_toolbox import read_from_file
from ppm_toolbox import read_from_device_uart
...
def ppm_measure(runcontinuosly=settings.runcontinuosly, plot=settings.plot):
	try:
		# start measurement
		if settings.demo:
			raw_data = read_from_file(settings.sampledatafile)
		else:
			raw_data = read_from_device_uart(settings.baudrate, settings.port, settings.samplerate)
